---
title: "Customer Segmentation in R"
...
library(readxl)
 customer_data=read_excel("D:/BEDIAM/Ebenezer/Logiciels/R/Machine_learning/Mall_Customers.xlsx")
 str(customer_data)
 names(customer_data)
...
##We will now display the first six rows of our dataset using the head() function and use the summary() function to output summary of it
 ...
 summary(customer_data$Annual_Income)
 sd(customer_data$Annual_Income)
 summary(customer_data$Spending_Score)
 sd(customer_data$Spending_Score)
```
...
#In this section of the R project, we will create visualizations to analyze the annual income of the customers. We will plot a histogram and then we will proceed to examine this data using a density plot
 ...
 summary(customer_data$Annual_Income)
 hist(customer_data$Annual_Income,
  col="#660033",
 ...
 plot(density(customer_data$Annual_Income),
    col="yellow",
    main="Density Plot for Annual Income",
    xlab="Annual Income Class",
    ylab="Density")
 polygon(density(customer_data$Annual_Income),
        col="#ccff66")
...
##Analyzing Spending Score of the Customers
 ...
 summary(customer_data$Spending_Score)
 boxplot(customer_data$Spending_Score,
   horizontal=TRUE,
   col="#990000",
   main="BoxPlot for Descriptive Analysis of Spending Score")
 
 hist(customer_data$Spending_Score,
...
#The first step is to indicate the number of clusters (k) that we wish to produce in the final output
...
```{r}
ggplot(customer_data, aes(x =Spending_Score, y =Age)) + 
  geom_point(stat = "identity", aes(color = as.factor(k6$cluster))) +
  scale_color_discrete(name=" ",
                      breaks=c("1", "2", "3", "4", "5","6"),
                      labels=c("Cluster 1", "Cluster 2", "Cluster 3", "Cluster 4", "Cluster 5","Cluster 6")) +
